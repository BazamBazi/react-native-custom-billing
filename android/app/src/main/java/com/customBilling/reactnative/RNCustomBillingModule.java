...
public class RNCustomBillingModule extends ReactContextBaseJavaModule implements ActivityEventListener {
    ...
    public void open(final Promise promise) {
        ...
        try {
            ...
            mHelper.startSetup(VENDOR_INTENT, VENDOR_PACKAGE, new IabHelper.OnIabSetupFinishedListener() {
                public void onIabSetupFinished(IabResult result) {
                    ...
                    if (result.isSuccess()) {
                        ...
                        if (mHelper == null) {
                            promise.reject(E_SETUP_DISCONNECT, "there is no connection to " + TAG);
                        } else {
                            // Oh noes, there was a problem.
                            promise.reject(E_SETUP_ERROR, "There is a problem in setting up" + TAG);
                        }
                    }
                }
            });
            ...
            promise.reject(E_SETUP_DISCONNECT, "there is no connection to " + TAG);
            Log.e(TAG + "OpenError", "Could not open " + TAG, excep);
        }
    }
    ...
    boolean verifyDeveloperPayload(Purchase p) {
        ...
        /*
         * TODO: verify that the developer payload of the purchase is correct. It will
         * be the same one that you sent when initiating the purchase.
         *
         * WARNING: Locally generating a random string when starting a purchase and
         * verifying it here might seem like a good approach, but this will fail in the
         * case where the user purchases an item on one device and then uses your app on
         * a different device, because on the other device you will not have access to
         * the random string you originally generated.
         *
         * So a good developer payload has these characteristics:
         *
         * 1. If two different users purchase an item, the payload is different between
         * them, so that one user's purchase can't be replayed to another user.
         *
         * 2. The payload must be such that you can verify it even when the app wasn't
         * the one who initiated the purchase flow (so that items purchased by the user
         * on one device work on other devices owned by the user).
         *
